  ...
  function insertNewMaterial(event) {
    ...
    materials?.forEach((material) => {
      if (material.name.toLowerCase() === newName.toLowerCase()) {
        ...
        console.log(
          `Quantidade atual = ${currentQuantity}; Nova quantidade = ${newQuantity}`
        );
      }
    });
    ...
    if (materialExists) {
      ...
      api
        .put(`/materials/${currentMaterialID}`, newMaterial)
        .then((response) => {
          console.log("Update status: " + response.status);
          description = `Material atualizado -> ${currentName}. Passou de -> ${currentQuantity}, para -> ${newQuantity}`;
          api
            .post("/movements", {
              user_id: user.id,
              user_name: user.name,
            ...
            .then((response) => {
              ...
              setStatus("Sucesso");
              setMessage(
                "O material já existia e foi atualizado e registrado com sucesso."
              );
              setVariant("success");
              setVisible(true);
            })
      ...
      api
        ...
        .then((response) => {
          ...
          description = `Material recebido -> ${newQuantity}x ${newName[0].toUpperCase() + newName.substring(1).toLowerCase()}.`;
          api
            .post("/movements", {
              user_id: user.id,
              user_name: user.name,
        ...
        <fieldset>
          ...
          >
            <Form.Control
              required={true}
              type="text"
              placeholder="Informe o nome do material"
            />
          </FloatingLabel>
          <FloatingLabel
